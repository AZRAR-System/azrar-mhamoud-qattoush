        ...
        try {
            ...
            Object.keys(data).forEach(key => {
                ...
                if (value !== undefined && value !== null && value !== '' && (typeof value === 'string' || typeof value === 'number' || typeof value === 'boolean')) {
                    ...
                    if(key.includes('id') || key.includes('Id') || key.includes('date') || key.includes('تاريخ')) return;
            ...
            Object.keys(data).forEach(key => {
                ...
                if (typeof val === 'number' && val > 0) {
                    ...
                    if (previousValues.length > 5) {
                        ...
                        if (val > mean * 2.5) {
                            anomalies.push(`القيمة المدخلة في "${key}" (${val}) أعلى بكثير من المعدل المعتاد (${Math.round(mean)}).`);
                        }
                    }
                }
            });
