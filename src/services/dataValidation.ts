...
export const checkUniqueConstraints = (): ValidationResult => {
  ...
  const usernames = users.map((u: any) => u.اسم_المستخدم);
  const duplicateUsernames = usernames.filter((name: string, index: number) => usernames.indexOf(name) !== index);
...
export const validateNewPerson = (data: Partial<الأشخاص_tbl>): ValidationResult => {
  ...
  if ((data as any).رقم_هاتف_اضافي && !isLikelyJordanPhone(String((data as any).رقم_هاتف_اضافي))) {
