const repeat = (s: string, times: number) =>
  Array.from({ length: Math.max(0, times) }, () => s).join('');
...
export const getPrintingQaSampleData = () => {
  ...
  const longCompany =
    'شركة أزرار للخدمات العقارية والاستشارات القانونية وإدارة الأملاك (فرع الشمال)';
  const longText =
