...
const getDesktopBridge = (): DesktopDbBridge | undefined =>
...
const deleteAttachmentFilesBestEffort = (attachments: Attachment[]) => {
  ...
  for (const a of attachments) {
    const p = asUnknownRecord(a)['filePath'];
...
const purgeRefs = (referenceType: Attachment['referenceType'], referenceId: string) => {
  // Attachments
...
const forceDeleteSalesAgreementInternal = (id: string): DbResult<null> => {
  ...
  // Clean up refs (if some components stored attachments/notes against Sales)
  purgeRefs('Sales', id);
...
const deletePersonCascadeInternal = (id: string): DbResult<null> => {
  ...
  if (oh.some((r) => r.رقم_المالك_القديم === id || r.رقم_المالك_الجديد === id)) {
    save(
      KEYS.OWNERSHIP_HISTORY,
      oh.filter((r) => !(r.رقم_المالك_القديم === id || r.رقم_المالك_الجديد === id))
    );
  }
...
export const DbService = {
  ...
  getSettings: (): SystemSettings => {
    ...
    try {
      ...
      // Clamp inactivity timeout to a sane range.
  ...
  saveSettings: (s: SystemSettings) => {
    ...
    // Update local cache immediately for sync readers.
  ...
  createSalesListing: (data: Partial<عروض_البيع_tbl>): DbResult<{ id: string }> => {
  ...
  // Sales agreement helpers (commissions + expenses payload from UI)
  updateSalesAgreement: (
    ...
    const expense =
      commissions?.expenses ?? patch.مصاريف_البيع ?? current.مصاريف_البيع;
    ...
    const commBuyer = Number(
      commissions?.buyer ?? patch.عمولة_المشتري ?? current.عمولة_المشتري ?? 0
    );
    const commSeller = Number(
      commissions?.seller ?? patch.عمولة_البائع ?? current.عمولة_البائع ?? 0
    );
