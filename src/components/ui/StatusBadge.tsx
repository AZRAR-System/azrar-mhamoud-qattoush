...
export const StatusBadge: React.FC<{ status: string; className?: string; showIcon?: boolean }> = ({
  ...
  const styles = {
    ...
    neutral:
      'bg-slate-100/70 text-slate-700 border-slate-200/80 dark:bg-slate-800/60 dark:text-slate-300 dark:border-slate-700',
  };
  ...
  return (
    ...
    >
      {showIcon && Icon ? (
        <span
          className={`inline-flex items-center justify-center w-4 h-4 rounded-full ${iconBg[config.type as StatusType]}`}
        >
