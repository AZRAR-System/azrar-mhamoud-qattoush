...
import type { UseOwnerPortalReturn, TabId } from '@/hooks/useOwnerPortal';
...
interface OwnerPortalPageViewProps {
  page: UseOwnerPortalReturn;
}

export const OwnerPortalPageView: React.FC<OwnerPortalPageViewProps> = ({ page }) => {
  const {
    activeTab, setActiveTab,
    report, loading,
    selectedOwnerId, setSelectedOwnerId,
    filterContractId, setFilterContractId,
    owners, searchTerm, setSearchTerm,
    filteredOwners,
            ...
            <div className="space-y-8 animate-fade-in">
              ...
              {Object.keys(report.byMonth).length === 0 ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
                  <div className="space-y-6">
                    ...
                    <div className="space-y-6">
                      {Object.entries(report.byMonth)
                        ...
                        .slice(0, 8)
                        .map(([month, data]) => {
                          const maxValue = Math.max(...Object.values(report.byMonth).map(m => m.expected), 1);
                          const collectedPct = (data.collected / maxValue) * 100;
                          const expectedPct = (data.expected / maxValue) * 100;
                          return (
                  ...
                  <div className="space-y-6">
                    <h4 className="text-sm font-black text-slate-500 flex items-center gap-2 uppercase tracking-wide">
                      <TrendingUp size={16} className="text-emerald-500" /> ملخص الربحية الصافية شهرياً
                    </h4>
