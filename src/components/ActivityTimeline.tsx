    ...
    <div className="relative border-r border-gray-200 dark:border-slate-700 mr-2 space-y-6">
      ...
      {activities.map((act) => (
        <div key={act.id} className="relative mr-6">
          <div
            className={`absolute top-0 -right-[31px] w-8 h-8 rounded-full border-4 border-white dark:border-slate-800 flex items-center justify-center ${getColor(act.actionType)}`}
          >
          ...
          <div className="bg-gray-50 dark:bg-slate-900/50 p-3 rounded-xl border border-gray-100 dark:border-slate-700">
            <div className="flex justify-between items-start mb-1">
              <span
                className={`text-[10px] font-bold px-2 py-0.5 rounded ${getColor(act.actionType)} bg-opacity-20 border-opacity-0`}
              >
                {act.actionType}
              </span>
