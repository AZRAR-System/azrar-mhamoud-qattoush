        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-2">
            {suggestions.map((s, idx) => (
              <span
                key={idx}
                className="inline-flex items-center gap-1 bg-white dark:bg-slate-800 px-2 py-1 rounded border border-indigo-100 dark:border-indigo-900 text-xs text-slate-600 dark:text-slate-300"
              >
                <span className="font-bold">{s.field}:</span> {String(s.suggestedValue)}
                <span className="text-[9px] text-green-500 opacity-80">
