    ...
    <div className={`flex justify-between items-center mb-3 ${isCustomizing ? 'handle cursor-move' : ''}`}>
        ...
        <div className="flex items-center gap-2">
            ...
            {isCustomizing && onRemove && (
                <button onClick={(e) => { e.stopPropagation(); onRemove(); }} className="text-red-500 hover:bg-red-50 rounded-full p-1"><X size={14} /></button>
            )}
        </div>
    </div>
    <div className="flex-1 min-h-0 overflow-y-auto custom-scrollbar relative flex flex-col">{children}</div>
  </div>
);
        ...
        <WidgetWrapper {...props} title="متابعة العملاء" icon={User}>
            <table className="w-full text-right text-[10px]">
                <thead className="text-slate-400 bg-slate-50 dark:bg-slate-900 sticky top-0">
                    <tr><th className="p-2">العميل</th><th className="p-2">الحدث</th><th className="p-2">التاريخ</th></tr>
                </thead>
...
export const DashboardCalendarWidget: React.FC<WidgetComponentProps> = (props) => {
    ...
    return (
        <WidgetWrapper {...props} title={`تقويم ${today.toLocaleDateString('ar-EG', { month: 'long' })}`} icon={Calendar}>
            <div className="grid grid-cols-7 gap-1 text-center text-[10px] h-full">
                {days.map(d => <div key={d} className="text-slate-400 font-bold py-1">{d}</div>)}
...
export const SmartRemindersWidget: React.FC<WidgetComponentProps> = (props) => {
    ...
    return (
        <WidgetWrapper {...props} title="تذكيرات ذكية" icon={Clock}>
            <form onSubmit={add} className="flex gap-2 mb-3">
                <input className="flex-1 bg-slate-50 dark:bg-slate-900 border rounded-lg px-2 py-1 text-xs outline-none" placeholder="تذكير جديد..." value={title} onChange={e=>setTitle(e.target.value)}/>
                <button type="submit" className="bg-purple-600 text-white p-1 rounded-lg"><Plus size={14}/></button>
            </form>
