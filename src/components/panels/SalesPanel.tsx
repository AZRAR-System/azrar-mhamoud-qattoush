...
export const SalesPanel: React.FC<{ id: string }> = ({ id }) => {
  ...
  const [desktopBuyerById, setDesktopBuyerById] = useState<Map<string, الأشخاص_tbl>>(
    () => new Map()
  );
  ...
  useEffect(() => {
    ...
    const run = async () => {
      const ids = Array.from(
        new Set(offers.map((o) => String(o.رقم_المشتري || '').trim()).filter(Boolean))
      );
  ...
  const propertyLegacy = useMemo(
    () =>
      isDesktopFast || desktopUnsupported
        ? undefined
        : listingPropertyId
          ? DbService.getProperties().find((p) => p.رقم_العقار === listingPropertyId)
          : undefined,
  ...
  const ownerLegacy = useMemo(
    () =>
      isDesktopFast || desktopUnsupported
        ? undefined
        : listingOwnerId
          ? DbService.getPeople().find((p) => p.رقم_الشخص === listingOwnerId)
          : undefined,
  ...
  const activeContractLegacy = useMemo(() => {
    ...
    return contracts.find(
      (c) => c.رقم_العقار === listingPropertyId && isTenancyRelevant(c) && c.isArchived !== true
    );
  }, [isDesktopFast, desktopUnsupported, listingPropertyId]);

  const property: العقارات_tbl | undefined = isDesktopFast
    ? (desktopProperty ?? undefined)
    : propertyLegacy;
  const owner: الأشخاص_tbl | undefined = isDesktopFast ? (desktopOwner ?? undefined) : ownerLegacy;
  const activeContract: العقود_tbl | undefined = isDesktopFast
    ? (desktopActiveContract ?? undefined)
    : activeContractLegacy;
      ...
      <div className="app-card p-6 relative overflow-hidden">
        ...
        <div className="flex justify-between items-start">
          ...
          <div className="text-left">
            ...
            <p className="text-2xl font-bold text-emerald-600">
              {listing.السعر_المطلوب.toLocaleString()} <span className="text-sm">د.أ</span>
            </p>
          </div>
        </div>
        ...
        <div className="mt-4 flex gap-4 text-sm text-slate-600 dark:text-slate-300 bg-gray-50 dark:bg-slate-900/50 p-3 rounded-xl">
          <span className="flex items-center gap-1">
            <User size={14} /> المالك: {owner?.الاسم}
          </span>
          <span className="text-gray-300">|</span>
          <span className="flex items-center gap-1">
            <Clock size={14} /> تاريخ العرض: {listing.تاريخ_العرض}
          </span>
          <span className="text-gray-300">|</span>
          <span
            className={`px-2 py-0.5 rounded text-xs font-bold ${listing.الحالة === 'Active' ? 'bg-green-100 text-green-700' : 'bg-orange-100 text-orange-700'}`}
          >
            {listing.الحالة}
          </span>
        </div>
      ...
      <div className="app-card overflow-hidden">
        ...
        {showOfferForm && canAddOffer && (
          <form
            ...
            className="p-4 bg-indigo-50 dark:bg-indigo-900/10 border-b border-indigo-100 dark:border-indigo-800 grid grid-cols-1 md:grid-cols-2 gap-4"
          >
            <div>
              <PersonPicker
                label="المشتري"
          ...
          {offers.length === 0 ? (
            ...
            offers.map((offer) => {
              ...
              const buyer = isDesktopFast
                ? buyerId
                  ? desktopBuyerById.get(buyerId)
                  : undefined
                : desktopUnsupported
                  ? undefined
                  : DbService.getPeople().find((b) => b.رقم_الشخص === offer.رقم_المشتري);
              return (
                <div
                  key={offer.id}
                  className="p-4 hover:bg-gray-50 dark:hover:bg-slate-700/30 transition"
                >
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <span className="text-xs text-slate-500">
                        {new Date(offer.تاريخ_العرض).toLocaleDateString()}
                      </span>
                      {buyer?.رقم_الشخص && (
                        <button
                    ...
                    <div className="text-right">
                      <span className="block font-bold text-emerald-600">
                        {offer.قيمة_العرض.toLocaleString()} د.أ
                      </span>
                      <span
                  ...
                  {offer.ملاحظات_التفاوض && (
                    <pre className="whitespace-pre-wrap text-xs text-slate-600 bg-gray-100 dark:bg-slate-900 p-2 rounded mt-2">
                      {offer.ملاحظات_التفاوض}
                    </pre>
                  )}
                  ...
                  {offer.الحالة === 'Pending' && (
                    <div className="mt-3 flex gap-2 justify-end">
                      <button
                        ...
                        className="text-red-600 bg-red-50 hover:bg-red-100 px-3 py-1 rounded text-xs font-bold flex items-center gap-1 transition"
                      >
                        <XCircle size={14} /> رفض
                      </button>
                      <button
                        onClick={() => handleStatusChange(offer.id, 'Accepted')}
                        className="text-green-600 bg-green-50 hover:bg-green-100 px-3 py-1 rounded text-xs font-bold flex items-center gap-1 transition"
                      >
