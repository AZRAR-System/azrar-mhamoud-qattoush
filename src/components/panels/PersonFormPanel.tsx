...
export const PersonFormPanel: React.FC<PersonFormProps> = ({ id, onClose, onSuccess, initialType }) => {
  ...
  return (
    <div className="p-6 h-full flex flex-col">
        <h2 className="text-2xl font-bold text-slate-800 dark:text-white mb-6 flex items-center gap-2">
                        ...
                        {id && id !== 'new'
                            ? (formData.نوع_الملف === 'منشأة' ? 'تعديل بيانات المنشأة' : 'تعديل بيانات الملف')
                            : (formData.نوع_الملف === 'منشأة' ? 'إضافة منشأة جديدة' : 'إضافة ملف جديد')}
        </h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 flex-1 overflow-y-auto">
            ...
            />
                        ...
                        <div className="p-4 bg-gray-50 dark:bg-slate-900/40 rounded-xl border border-gray-200 dark:border-slate-700">
                                <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">نوع الملف</label>
                                <div className="flex bg-gray-100 dark:bg-slate-900 p-1 rounded-xl w-full overflow-x-auto no-scrollbar">
                                        <button
                                            type="button"
                                            onClick={() => setFormData(prev => ({ ...prev, نوع_الملف: 'فرد', طبيعة_الشركة: '' }))}
            ...
            <div className="p-4 bg-indigo-50 dark:bg-indigo-900/10 rounded-xl border border-indigo-100 dark:border-indigo-800">
                <div className="flex justify-between items-center mb-3">
                    <label className="text-sm font-bold text-indigo-800 dark:text-indigo-300">تصنيف الأدوار</label>
                    <button type="button" onClick={() => setIsAddingRole(true)} className="text-xs bg-white dark:bg-slate-800 px-2 py-1 rounded shadow-sm text-indigo-600 hover:text-indigo-700">
                        <Plus size={12} className="inline mr-1"/> جديد
                    </button>
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                    {availableRoles.map(r => (
                        <label key={r.id} className={`cursor-pointer px-3 py-1.5 rounded-lg text-sm border transition select-none flex items-center gap-2
                        ...
                        `}>
                            <input type="checkbox" className="hidden" checked={formData.selectedRoles.has(r.label)} onChange={() => toggleRole(r.label)} />
                            {r.label}
                        </label>
                    ))}
                </div>
            ...
            <div className="pt-4 flex gap-3 border-t border-gray-100 dark:border-slate-700">
                <button type="button" onClick={onClose} className="flex-1 py-3 text-slate-500 font-bold hover:bg-gray-100 dark:hover:bg-slate-800 rounded-xl transition">إلغاء</button>
                <button type="submit" className="flex-[2] py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-600/20 transition">حفظ البيانات</button>
            </div>
        </form>
    </div>
  );
