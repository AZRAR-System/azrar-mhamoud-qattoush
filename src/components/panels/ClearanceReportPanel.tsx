...
export const ClearanceReportPanel: React.FC<{ id: string }> = ({ id }) => {
  ...
  if (!record)
    return (
      <div className="p-10 text-center text-red-500">لم يتم العثور على سجل مخالصة لهذا العقد.</div>
    );
        ...
        <div className="max-w-3xl mx-auto bg-white p-10 shadow-lg print:shadow-none print:max-w-full text-black">
          ...
          <div className="mb-8">
            ...
            <table className="w-full text-right border-collapse text-sm">
              ...
              <tbody>
                <tr>
                  <td className="border p-2">إيجارات متأخرة</td>
                  <td className="border p-2 font-bold">{record.rentArrears}</td>
                </tr>
                <tr>
                  <td className="border p-2">ذمم فواتير (كهرباء ومياه)</td>
                  <td className="border p-2 font-bold">
                    {record.electricity.amountDue + record.water.amountDue}
                  </td>
                </tr>
                <tr>
                  <td className="border p-2">أضرار وصيانة</td>
                  <td className="border p-2 font-bold">
                    {record.damages.reduce((sum, d) => sum + d.cost, 0)}
                  </td>
                </tr>
                <tr>
                  <td className="border p-2">رسوم تنظيف / أخرى</td>
                  <td className="border p-2 font-bold">{record.cleaningFee}</td>
                </tr>
          ...
          <div className="border-2 border-black p-6 rounded-xl mb-12 bg-gray-50">
            ...
            <p className="text-sm leading-7 text-justify">
              أقر أنا الموقع أدناه، بصفتي المستأجر للعقار المذكور أعلاه، بأنني قمت بتسليم العقار
              بتاريخ {record.date}، وتم إجراء المخالصة المالية والفنية حسب التفاصيل الواردة في هذا
              التقرير. وبناءً عليه:
            </p>
