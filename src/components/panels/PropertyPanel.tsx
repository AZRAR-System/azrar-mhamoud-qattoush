...
import { Home, MapPin, User, Zap, Droplets, FileText, ArrowRight, History, Edit2, Trash2, Printer, Briefcase, ClipboardCheck, ListTodo } from 'lucide-react';
    ...
    useEffect(() => {
        ...
        if (!isDesktopFast) {
            ...
            try {
                ...
                const inspector = latestInspection?.inspectorId ? people.find((x) => x.رقم_الشخص === latestInspection.inspectorId) : null;
                const client = latestInspection?.clientId ? people.find((x) => x.رقم_الشخص === latestInspection.clientId) : null;
  ...
  return (
    <div className="space-y-6">
                        ...
                        <table className="w-full text-right text-sm">
                            <tbody className="divide-y divide-gray-200">
                                ...
                                <tr>
                                    ...
                                    <td className="p-3">{p.النوع || '—'} • {p.حالة_العقار || '—'}</td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td className="p-3">{owner?.الاسم || 'غير معروف'} {owner?.رقم_الهاتف ? `• ${owner.رقم_الهاتف}` : ''}</td>
                                </tr>
                                {currentContract ? (
                                    <tr>
                                        <td className="p-3 bg-gray-50 font-bold">العقد الحالي</td>
                                        <td className="p-3">#{formatContractNumberShort(currentContract.رقم_العقد)} • {currentContract.تاريخ_البداية} → {currentContract.تاريخ_النهاية}</td>
                                    </tr>
                                ) : null}
                                {currentTenant ? (
                                    <tr>
                                        <td className="p-3 bg-gray-50 font-bold">المستأجر الحالي</td>
                                        <td className="p-3">{currentTenant.الاسم || '—'} {currentTenant.رقم_الهاتف ? `• ${currentTenant.رقم_الهاتف}` : ''}</td>
                                    </tr>
                                ) : null}
                                {currentGuarantor ? (
                                    <tr>
                                        <td className="p-3 bg-gray-50 font-bold">الكفيل</td>
                                        <td className="p-3">{currentGuarantor.الاسم || '—'} {currentGuarantor.رقم_الهاتف ? `• ${currentGuarantor.رقم_الهاتف}` : ''}</td>
                                    </tr>
                                ) : null}
        ...
        <div className="flex gap-4 items-start">
           ...
           <div className="flex-1">
              <h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
                 {p.الكود_الداخلي}
                 <span className={`text-xs px-2 py-1 rounded-full ${p.IsRented ? 'bg-red-100 text-red-600' : 'bg-green-100 text-green-600'}`}>
                    {p.حالة_العقار}
                 </span>
              </h1>
      ...
      <div className="flex gap-2 p-1 bg-gray-100 dark:bg-slate-900 rounded-xl w-fit">
            <button onClick={() => setActiveTab('info')} className={`px-4 py-2 rounded-lg text-sm font-bold transition ${activeTab === 'info' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600' : 'text-gray-500'}`}>
            المعلومات والتفاصيل
         </button>
            <button onClick={() => setActiveTab('activity')} className={`px-4 py-2 rounded-lg text-sm font-bold transition ${activeTab === 'activity' ? 'bg-white dark:bg-slate-700 shadow text-indigo-600' : 'text-gray-500'}`}>
      ...
      {activeTab === 'info' ? (
        <div className="space-y-6 animate-fade-in">
                ...
                <div className="flex items-start justify-between gap-3">
                    <div>
                        <h3 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                            <ClipboardCheck size={18} className="text-indigo-600" /> الكشف (آخر كشف + آخر صور)
                        </h3>
                        <div className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                            {latestInspection ? `تاريخ: ${latestInspection.inspectionDate}` : 'لا يوجد كشف بعد'}
                        </div>
                    </div>

                    <button
                        type="button"
                        onClick={() => openPanel('INSPECTION_FORM', latestInspection?.id, { propertyId: p.رقم_العقار, onSuccess: reload })}
                ...
                {latestInspection ? (
                    <div className="mt-4 space-y-3">
                        <div className="flex flex-wrap gap-2 text-xs">
                            <span className={`px-2 py-1 rounded-full border ${latestInspection.isReady ? 'bg-green-50 text-green-700 border-green-200 dark:bg-green-900/10 dark:text-green-300 dark:border-green-800' : 'bg-yellow-50 text-yellow-700 border-yellow-200 dark:bg-yellow-900/10 dark:text-yellow-300 dark:border-yellow-800'}`}>
                                    ...
                                    {inspectionsForProperty.slice(1).map((ins) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex items-center gap-2 flex-shrink-0">
                                                <button
                                                    type="button"
                                                    onClick={() => openPanel('INSPECTION_FORM', ins.id, { propertyId: p.رقم_العقار, onSuccess: reload })}
            ...
            {/* Owner Info */}
            <div className="bg-indigo-50 dark:bg-indigo-900/10 p-4 rounded-xl border border-indigo-100 dark:border-indigo-800 flex justify-between items-center cursor-pointer hover:bg-indigo-100 dark:hover:bg-indigo-900/20 transition"
                onClick={() => owner && openPanel('PERSON_DETAILS', owner.رقم_الشخص)}>
                <div className="flex items-center gap-3">
                    <div className="bg-indigo-200 dark:bg-indigo-800 p-2 rounded-full text-indigo-700 dark:text-indigo-200"><User size={16}/></div>
                    <div>
                    <p className="text-xs text-indigo-600 dark:text-indigo-400 font-bold">المالك</p>
                    <p className="text-sm font-bold text-slate-800 dark:text-white">{owner?.الاسم || 'غير معروف'}</p>
                    </div>
                </div>
            ...
            {ownershipHistory.length > 0 && (
                <div className="app-card p-4">
                    ...
                    <div className="space-y-2">
                        {ownershipHistory
                            .slice()
                            .sort((a: سجل_الملكية_tbl, b: سجل_الملكية_tbl) => String(b.تاريخ_نقل_الملكية || '').localeCompare(String(a.تاريخ_نقل_الملكية || '')))
                            .slice(0, 20)
                            .map((r: سجل_الملكية_tbl) => (
                                <div key={r.id} className="p-3 bg-gray-50 dark:bg-slate-900/50 rounded-lg border border-gray-100 dark:border-slate-700">
                                    ...
                                    <div className="text-xs text-slate-600 dark:text-slate-300 mt-1">
                                        {typeof r.السعر_النهائي === 'number' ? `السعر النهائي: ${r.السعر_النهائي.toLocaleString()} د.أ` : ''}
                                    </div>
                                </div>
                            ))}
                    </div>
            ...
            {currentContract && (
                <div className="bg-purple-50 dark:bg-purple-900/10 p-4 rounded-xl border border-purple-100 dark:border-purple-800">
                    <div className="flex justify-between items-center">
                        <div className="flex items-center gap-3">
                            <div className="bg-purple-200 dark:bg-purple-800 p-2 rounded-full text-purple-700 dark:text-purple-200"><User size={16}/></div>
                            <div>
                                <p className="text-xs text-purple-600 dark:text-purple-400 font-bold">المستأجر الحالي</p>
                                <p className="text-sm font-bold text-slate-800 dark:text-white">{currentTenant?.الاسم || 'غير معروف'}</p>
                                {(currentTenant?.رقم_الهاتف || currentContract?.تاريخ_النهاية) && (
                                    <div className="text-[11px] text-slate-600 dark:text-slate-300 mt-0.5 flex flex-wrap gap-x-3 gap-y-1">
                                        {currentTenant?.رقم_الهاتف ? <span>هاتف: {currentTenant.رقم_الهاتف}</span> : null}
                                        {currentContract?.تاريخ_النهاية ? <span>نهاية العقد: {currentContract.تاريخ_النهاية}</span> : null}
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    {currentGuarantor ? (
                        <div className="mt-3 pt-3 border-t border-purple-100 dark:border-purple-800/60 text-[11px] text-slate-700 dark:text-slate-200">
                            ...
                            {currentGuarantor.رقم_الهاتف ? <span className="text-slate-600 dark:text-slate-300 dir-ltr"> • {currentGuarantor.رقم_الهاتف}</span> : null}
                        </div>
                    ) : null}
                </div>
            )}
            ...
            <div className="grid grid-cols-2 gap-4">
                {p.رقم_اشتراك_الكهرباء && (
                    ...
                    <p className="text-sm font-mono text-slate-700 dark:text-slate-300">{p.رقم_اشتراك_الكهرباء}</p>
                    </div>
                )}
                {p.رقم_اشتراك_المياه && (
                    ...
                    <p className="text-sm font-mono text-slate-700 dark:text-slate-300">{p.رقم_اشتراك_المياه}</p>
                    </div>
                )}
            </div>
            ...
            <div className="app-card p-4">
                ...
                <div className="space-y-2">
                    {allContractsForProperty.length === 0 ? (
                        <div className="text-sm text-slate-500 dark:text-slate-400">لا توجد عقود لهذا العقار</div>
                    ) : (
                                    ...
                                    <div className="flex justify-between items-start gap-3">
                                        <div className="min-w-0">
                                            <div className="text-xs mb-1 flex flex-wrap items-center gap-2">
                                                <span className={`font-bold ${labelClass}`}>{isActive ? (isCurrent ? 'عقد حالي' : 'عقد نشط') : 'منتهي'}</span>
                                                <span className="text-[10px] bg-white/70 dark:bg-slate-800 border px-2 py-0.5 rounded">{String(c.حالة_العقد || '')}</span>
                                                {c.تاريخ_البداية || c.تاريخ_النهاية ? (
                                                    <span className="text-[11px] text-slate-500 dark:text-slate-400">
                                                        {String(c.تاريخ_البداية || '')}{c.تاريخ_النهاية ? ` → ${c.تاريخ_النهاية}` : ''}
                                                    </span>
                                                ) : null}
                                            </div>
                                            ...
                                            <div className="text-[11px] text-slate-600 dark:text-slate-300 mt-1 flex flex-wrap gap-x-3 gap-y-1">
                                                <span>
                                                    المستأجر: <span className="font-semibold">{isDesktopFast ? (tenantNameFast || '—') : (tenant?.الاسم || '—')}</span>
                                                </span>
                                                <span>
                                                    الكفيل: <span className="font-semibold">{isDesktopFast ? (guarantorNameFast || '—') : (guarantor?.الاسم || '—')}</span>
                                                </span>
                                            </div>
                                        </div>
                ...
                <div className="app-card p-4">
                    ...
                    <div className="text-sm text-slate-700 dark:text-slate-200 space-y-2">
                        ...
                        <div className="flex justify-between">
                            ...
                            <b className="text-emerald-600">{Number(openSalesListing.السعر_المطلوب || 0).toLocaleString()} د.أ</b>
                        </div>
                        ...
                        {openSalesListing.تاريخ_العرض ? (
                            <div className="text-xs text-slate-500">تاريخ العرض: <b>{openSalesListing.تاريخ_العرض}</b></div>
                        ) : null}
                    </div>
                </div>
            ) : null}
            ...
            {latestSaleAgreement?.a && (
                <div className="app-card p-4">
                    <div className="flex items-center justify-between mb-4">
                        ...
                        <div className="flex gap-2">
                            <button
                                onClick={() => handleEditAgreementFromProperty(String(latestSaleAgreement.a.id))}
                            ...
                            <button
                                onClick={() => handleDeleteAgreementFromProperty(String(latestSaleAgreement.a.id))}
                    ...
                    <div className="text-sm text-slate-700 dark:text-slate-200 space-y-2">
                        <div className="flex justify-between">
                            ...
                            <b className="text-emerald-600">{Number(latestSaleAgreement.a.السعر_النهائي || 0).toLocaleString()} د.أ</b>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="text-xs text-slate-500 bg-gray-50 dark:bg-slate-900/40 p-3 rounded-lg border border-gray-100 dark:border-slate-700">
                            <div>عمولة البائع: <b>{Number(latestSaleAgreement.a.عمولة_البائع || 0).toLocaleString()}</b> • عمولة المشتري: <b>{Number(latestSaleAgreement.a.عمولة_المشتري || 0).toLocaleString()}</b> • وسيط خارجي: <b>{Number(latestSaleAgreement.a.عمولة_وسيط_خارجي || 0).toLocaleString()}</b></div>
                            {latestSaleAgreement.a.مصاريف_البيع && (
                                <div className="mt-1">
                                    رسوم التنازل: <b>{Number(latestSaleAgreement.a.مصاريف_البيع.رسوم_التنازل || 0).toLocaleString()}</b> • ضريبة الأبنية: <b>{Number(latestSaleAgreement.a.مصاريف_البيع.ضريبة_الابنية || 0).toLocaleString()}</b>
                                    <br />
                                    نقل كهرباء: <b>{Number(latestSaleAgreement.a.مصاريف_البيع.نقل_اشتراك_الكهرباء || 0).toLocaleString()}</b> • نقل مياه: <b>{Number(latestSaleAgreement.a.مصاريف_البيع.نقل_اشتراك_المياه || 0).toLocaleString()}</b> • تأمينات: <b>{Number(latestSaleAgreement.a.مصاريف_البيع.قيمة_التأمينات || 0).toLocaleString()}</b>
                                </div>
                            )}
                        </div>
                        ...
                        {latestSaleAgreement.a.transactionId && (
                            <div className="text-xs text-slate-500">رقم معاملة دائرة الأراضي: <b>{latestSaleAgreement.a.transactionId}</b></div>
                        )}

                        <div className="flex gap-2 pt-2">
                            {latestSaleAgreement.sellerId && (
                                <button onClick={() => openPanel('PERSON_DETAILS', latestSaleAgreement.sellerId)} className="text-xs font-bold text-indigo-600 hover:underline">ملف البائع</button>
                            )}
                            {latestSaleAgreement.a.رقم_المشتري && (
                                <button onClick={() => openPanel('PERSON_DETAILS', latestSaleAgreement.a.رقم_المشتري)} className="text-xs font-bold text-indigo-600 hover:underline">ملف المشتري</button>
                            )}
                        </div>
                    </div>
                </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-slide-up h-full">
            <div className="bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-200 dark:border-slate-700 overflow-y-auto max-h-[500px] custom-scrollbar">
                <h3 className="font-bold text-lg mb-4 flex items-center gap-2"><History className="text-orange-500" size={20}/> سجل التطورات</h3>
