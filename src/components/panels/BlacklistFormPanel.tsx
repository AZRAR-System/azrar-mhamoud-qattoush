...
export const BlacklistFormPanel: React.FC<BlacklistFormProps> = ({
  id,
  mode = 'create',
  onClose,
  onSuccess,
    ...
    <div className="flex flex-col h-full bg-white dark:bg-slate-800">
      <div className="p-6 bg-red-50 dark:bg-red-900/20 border-b border-red-100 dark:border-red-900">
        <h2 className="text-xl font-bold text-red-700 dark:text-red-400 flex items-center gap-2">
          <ShieldAlert size={24} />{' '}
          {mode === 'create' ? 'إضافة للقائمة السوداء' : 'تعديل سجل الحظر'}
        </h2>
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-6 flex-1">
        ...
        <div>
          <label className="block text-sm font-bold text-slate-700 dark:text-slate-300 mb-2">
            سبب الحظر / المشكلة <span className="text-red-500">*</span>
          </label>
          <textarea
            required
      ...
      <div className="p-4 border-t border-gray-100 dark:border-slate-700 flex justify-end gap-3 bg-gray-50 dark:bg-slate-900/50">
        <button
          type="button"
          onClick={onClose}
          className="px-6 py-2.5 rounded-xl text-slate-600 font-bold hover:bg-gray-200 transition"
        >
          إلغاء
        </button>
        <button
          onClick={handleSubmit}
          className="px-6 py-2.5 rounded-xl bg-red-600 text-white font-bold hover:bg-red-700 shadow-lg shadow-red-600/20 flex items-center gap-2"
        >
