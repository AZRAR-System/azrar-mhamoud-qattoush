...
export const CalendarEventsPanel: React.FC<CalendarEventsPanelProps> = ({ id }) => {
  ...
  const reload = useCallback(async () => {
    ...
    if (isDesktopFast) {
      ...
      exp
        .filter((r) => String(r?.endDate || '').slice(0, 10) === date)
        .forEach((r) => {
          loadedEvents.push({
            type: 'Contract',
            time: '12:00',
      ...
      installments.forEach((i) =>
        loadedEvents.push({
          type: 'Installment',
          time: '09:00',
          title: `دفعة مستحقة: ${i.القيمة} د.أ`,
          details: `عقد #${i.رقم_العقد}`,
      ...
      contracts.forEach((c) =>
        loadedEvents.push({
          type: 'Contract',
          time: '12:00',
          title: `انتهاء عقد #${formatContractNumberShort(c.رقم_العقد)}`,
          details: 'يرجى مراجعة التجديد',
          status: 'Expiring',
        })
      );
    }
    ...
    const propertyById = new Map<string, (typeof properties)[number]>(
      properties.map((p) => [String(p.رقم_العقار), p])
    );
    followUps.forEach((f) => {
      ...
      const contractLine = linkedContractId
        ? `عقد #${formatContractNumberShort(linkedContractId)}`
        : '';
  ...
  const addTaskForDay = async () => {
    ...
    if (phone) {
      ...
      if (isDesktop) {
        ...
        exists = people.some(
          (p) =>
            normalizePhone(p.رقم_الهاتف) === normalized ||
            normalizePhone(p.رقم_هاتف_اضافي) === normalized
        );
      }
      ...
      <div className="space-y-4">
        {events.length === 0 ? (
          ...
          events.map((evt, idx) => {
            ...
            const isPending =
              String(evt?.status) !== 'Done' &&
              String(evt?.status) !== 'Paid' &&
              String(evt?.status) !== 'completed';
            const isOverdueNow =
              isToday && isPending && evtMinutes !== null && nowMinutes >= evtMinutes;
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-3">
                    <h4 className="font-bold text-slate-700 dark:text-white break-words">
                      {evt.title}
                    </h4>
                    <div
                      className="text-[10px] text-slate-500 dark:text-slate-400 flex items-center gap-1 flex-shrink-0"
                      dir="ltr"
                    >
                      <Clock size={12} />
                  ...
                  {evt.details && (
                    <p className="text-xs text-slate-500 break-words">{evt.details}</p>
                  )}
