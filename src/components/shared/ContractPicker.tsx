...
export const ContractPicker: React.FC<ContractPickerProps> = ({
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      const prop = contract?.رقم_العقار
        ? await domainGetSmart('properties', String(contract.رقم_العقار))
        : null;
      const tenant = contract?.رقم_المستاجر
        ? await domainGetSmart('people', String(contract.رقم_المستاجر))
        : null;
    ...
    <div className="w-full">
      ...
      <div
        ...
        className={
          ...
          (value
            ? 'border-indigo-200/70 dark:border-slate-700'
            : 'border-slate-200/80 dark:border-slate-700')
        }
      >
        <div className="flex items-center gap-3 min-w-0">
          <div
            className={`p-2 rounded-lg ${selectedContract ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300' : 'bg-slate-100 text-slate-400 dark:bg-slate-800/60 dark:text-slate-500'}`}
          >
            <FileText size={18} />
          </div>
          <div className="flex flex-col min-w-0">
            <span
              className={`font-bold whitespace-normal break-words ${selectedContract ? 'text-slate-800 dark:text-white' : 'text-slate-400'}`}
            >
              {selectedContract
            ...
            {selectedContract ? (
              <span className="text-[10px] text-slate-500 flex flex-wrap items-start gap-2">
                <span className="min-w-0 whitespace-normal break-words">
                  {selectedTenant?.الاسم || 'مستأجر غير معروف'}
                </span>
                <span className="opacity-70">|</span>
                <span className="whitespace-normal break-words">
                  نهاية: {formatDateYMD(selectedContract.تاريخ_النهاية)}
                </span>
              </span>
            ) : null}
          </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  بحث: الكود الداخلي، الرقم الوطني، اسم المالك/المستأجر، رقم العقد
                </div>
              </div>
            </div>
          }
