...
export const PersonPicker: React.FC<PersonPickerProps> = ({
  ...
  const openAddView = () => {
    const nextRole: PersonRole =
      defaultRole ?? (activeRoleFilter !== 'All' ? activeRoleFilter : 'مستأجر');
  ...
  const getRoleBadgeColor = (role: string) => {
    switch (role) {
      ...
      case 'كفيل':
        return 'bg-orange-100 text-orange-700 border-orange-200 dark:bg-orange-500/10 dark:text-orange-300 dark:border-orange-500/20';
      case 'مشتري':
    ...
    <div className="w-full">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <div
            className={`p-1.5 rounded-full ${selectedPerson ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300' : 'bg-slate-100 text-slate-400 dark:bg-slate-800/60 dark:text-slate-500'}`}
          >
            <User size={16} />
          </div>
          <div className="flex flex-col min-w-0">
            <span
              className={`font-bold whitespace-normal break-words ${selectedPerson ? 'text-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}
            >
              {selectedPerson ? selectedPerson.الاسم : placeholder}
            </span>
            {selectedPerson && (
              <span className="text-[10px] text-slate-500 flex flex-wrap items-start gap-2">
                <span className="whitespace-normal break-words dir-ltr font-mono tabular-nums">
                  {selectedPerson.رقم_الهاتف}
                </span>
                {selectedPerson.الرقم_الوطني && (
                    ...
                    >
                      <thead className="bg-slate-100/70 dark:bg-slate-950/40 text-slate-600 dark:text-slate-300 text-xs font-bold sticky top-0 z-10 shadow-sm whitespace-nowrap [overflow-wrap:normal] [word-break:normal]">
                        <tr>
                          ...
                          <th className="px-4 py-3 whitespace-nowrap text-center w-[18%]">
                            الهاتف
                          </th>
                          <th className="px-4 py-3 whitespace-nowrap text-center hidden md:table-cell w-[18%]">
                            الرقم الوطني
                          </th>
                          <th className="px-4 py-3 whitespace-nowrap text-center w-[16%]">
                            الأدوار
                          </th>
                          ...
                          return (
                            ...
                            >
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-3 min-w-0">
                                  <div
                                    className={`w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800/60 flex items-center justify-center text-slate-600 dark:text-slate-300 font-bold flex-shrink-0 ${isContractLinked ? 'ring-2 ring-emerald-400/60 dark:ring-emerald-400/40' : ''}`}
                                  >
                                    {String(p.الاسم || '')
                                      .trim()
                                      .charAt(0) || '؟'}
                              ...
                              <td className="px-4 py-3 text-center">
                                <div className="flex gap-1 flex-wrap justify-center">
                                  {(roles?.length ? roles : ['—']).map((r) => (
                                    <span
                                      key={r}
                                      className={`px-2 py-0.5 rounded text-[10px] font-bold border whitespace-nowrap ${getRoleBadgeColor(r)}`}
                                    >
                                      {r}
                                    </span>
                                  ))}
              ...
              <form onSubmit={handleAddPerson} className="flex-1 flex flex-col">
                <div className="flex-1 overflow-y-auto p-6 space-y-4">
                  <div className="bg-indigo-50/70 dark:bg-indigo-500/10 p-4 rounded-xl text-sm text-indigo-800 dark:text-indigo-300 mb-4 border border-indigo-200/70 dark:border-indigo-500/20">
                    سيتم إضافة الشخص الجديد بالدور:{' '}
                    <span className="font-bold">{newPersonRole}</span>. يمكنك تعديل الأدوار لاحقاً
                    من إدارة الأشخاص.
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      required
                      className="w-full border border-slate-200/80 dark:border-slate-800 p-3 rounded-xl bg-gray-50 dark:bg-slate-900 text-sm outline-none focus:ring-2 focus:ring-indigo-500/35 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-950 text-slate-900 dark:text-slate-100"
                      value={newPersonForm.الاسم}
                      onChange={(e) =>
                        setNewPersonForm({ ...newPersonForm, الاسم: e.target.value })
                      }
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        required
                        className="w-full border border-slate-200/80 dark:border-slate-800 p-3 rounded-xl bg-gray-50 dark:bg-slate-900 text-sm outline-none focus:ring-2 focus:ring-indigo-500/35 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-950 text-slate-900 dark:text-slate-100"
                        value={newPersonForm.رقم_الهاتف}
                        onChange={(e) =>
                          setNewPersonForm({ ...newPersonForm, رقم_الهاتف: e.target.value })
                        }
                      />
                    </div>
                    <div>
                      ...
                      <input
                        className="w-full border border-slate-200/80 dark:border-slate-800 p-3 rounded-xl bg-gray-50 dark:bg-slate-900 text-sm outline-none focus:ring-2 focus:ring-indigo-500/35 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-950 text-slate-900 dark:text-slate-100"
                        value={newPersonForm.الرقم_الوطني}
                        onChange={(e) =>
                          setNewPersonForm({ ...newPersonForm, الرقم_الوطني: e.target.value })
                        }
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      className="w-full border border-slate-200/80 dark:border-slate-800 p-3 rounded-xl bg-gray-50 dark:bg-slate-900 text-sm outline-none focus:ring-2 focus:ring-indigo-500/35 focus:ring-offset-2 focus:ring-offset-white dark:focus:ring-offset-slate-950 text-slate-900 dark:text-slate-100"
                      value={newPersonForm.العنوان}
                      onChange={(e) =>
                        setNewPersonForm({ ...newPersonForm, العنوان: e.target.value })
                      }
                    />
                  </div>
                </div>
                ...
                <div className="p-4 border-t border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 flex gap-3">
                  <button
                    ...
                    className="flex-1 py-3 text-slate-600 bg-gray-100 hover:bg-gray-200 rounded-xl font-bold transition"
                  >
                    رجوع للقائمة
                  </button>
                  <button
                    type="submit"
                    className="flex-[2] py-3 bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold transition shadow-lg shadow-green-600/20"
                  >
                    حفظ واختيار
                  </button>
                </div>
