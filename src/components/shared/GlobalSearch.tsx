        ...
        <div className="modal-overlay app-modal-overlay z-[100] items-start pt-20 transition-all animate-fade-in"
          ...
          >
            ...
            <div className="flex items-center px-4 py-4 border-b border-slate-200/70 dark:border-slate-800 gap-3 bg-slate-50/70 dark:bg-slate-950/30">
              ...
              />
              <button onClick={() => setIsOpen(false)} className="p-1.5 bg-slate-100/80 dark:bg-slate-800/60 rounded-lg text-slate-600 dark:text-slate-300 hover:text-rose-600 dark:hover:text-rose-400">
              ...
              {results.people.length > 0 && (
                <div className="p-2">
                  ...
                  {results.people.map(p => (
                    ...
                    >
                      <div className="p-2 bg-white dark:bg-slate-900 rounded-full text-indigo-600 dark:text-indigo-300 shadow-sm ring-1 ring-black/5 dark:ring-white/5"><User size={18}/></div>
                ...
                <div className="p-2">
                  ...
                  {results.properties.map(p => (
                    <div 
                      ...
                      className="flex items-center gap-3 p-3 rounded-xl hover:bg-indigo-50/70 dark:hover:bg-indigo-500/10 cursor-pointer group transition"
                    >
                      <div className="p-2 bg-white dark:bg-slate-900 rounded-full text-indigo-600 dark:text-indigo-300 shadow-sm ring-1 ring-black/5 dark:ring-white/5"><Home size={18}/></div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                ...
                <div className="p-2">
                  ...
                  {results.contracts.map(c => (
                    <div 
                      ...
                      className="flex items-center gap-3 p-3 rounded-xl hover:bg-indigo-50/70 dark:hover:bg-indigo-500/10 cursor-pointer group transition"
                    >
                      <div className="p-2 bg-white dark:bg-slate-900 rounded-full text-indigo-600 dark:text-indigo-300 shadow-sm ring-1 ring-black/5 dark:ring-white/5"><FileText size={18}/></div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
            ...
            <div className="bg-slate-50/70 dark:bg-slate-950/30 p-3 text-center border-t border-slate-200/70 dark:border-slate-800">
               <p className="text-xs text-slate-400">
                 استخدم <kbd className="font-sans bg-white/70 dark:bg-slate-900/40 px-1 rounded border border-slate-200/70 dark:border-slate-800">↑</kbd> <kbd className="font-sans bg-white/70 dark:bg-slate-900/40 px-1 rounded border border-slate-200/70 dark:border-slate-800">↓</kbd> للتنقل، و <kbd className="font-sans bg-white/70 dark:bg-slate-900/40 px-1 rounded border border-slate-200/70 dark:border-slate-800">Enter</kbd> للاختيار
               </p>
            </div>
          </div>
        </div>
