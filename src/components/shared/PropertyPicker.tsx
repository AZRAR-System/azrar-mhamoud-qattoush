...
const getRecordProp = (record: Record<string, unknown> | null, key: string): unknown =>
...
export const PropertyPicker: React.FC<PropertyPickerProps> = ({
  ...
  const useSqlRows =
    isDesktop && typeof window.desktopDb?.domainPropertyPickerSearch === 'function';
  ...
  const isPropertyForSale = (p: العقارات_tbl) =>
    Boolean(p.isForSale) || String(p.حالة_العقار || '').trim() === 'معروض للبيع';
  ...
  const filteredProperties = useMemo(() => {
    ...
    if (filterStatus === 'شاغر') {
      const activePropertyIds = new Set(
        contracts.filter((c) => isTenancyRelevant(c)).map((c) => c.رقم_العقار)
      );
    ...
    <div className="w-full">
      ...
      >
        <div className="flex items-center gap-3 min-w-0">
          <div
            className={`p-2 rounded-lg ${selectedProperty ? 'bg-indigo-100 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-300' : 'bg-slate-100 text-slate-400 dark:bg-slate-800/60 dark:text-slate-500'}`}
          >
            <Home size={18} />
          </div>
          <div className="flex flex-col min-w-0">
            <span
              className={`font-bold whitespace-normal break-words ${selectedProperty ? 'text-slate-800 dark:text-white' : 'text-slate-500 dark:text-slate-400'}`}
            >
              {selectedProperty
            ...
            {selectedProperty && (
              <span className="text-[10px] text-slate-500 flex items-start gap-2 flex-wrap">
                <span className="min-w-0 whitespace-normal break-words">
                  {selectedProperty.العنوان}
                </span>
                <StatusBadge
                  status={selectedProperty.حالة_العقار}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="text-xs text-slate-500 dark:text-slate-400">
                  ابحث واختر العقار المناسب للعملية
                </div>
              </div>
            </div>
          }
        >
          ...
          <div className="p-4 space-y-4 bg-white dark:bg-slate-900 border-b border-slate-200/70 dark:border-slate-800">
            <div className="flex gap-3">
              <div className="relative flex-1">
                ...
                />
                <Search
                  className={`absolute top-3 text-gray-400 ${isRtl ? 'left-4' : 'right-4'}`}
                  size={20}
                />
              </div>
          ...
          <div className="flex-1 overflow-y-auto bg-gray-50 dark:bg-slate-900/50 p-2 custom-scrollbar">
            {desktopUnsupported ? (
              ...
              <>
                ...
                {!isLoading &&
                (useSqlRows ? filteredPickerRows.length === 0 : filteredProperties.length === 0) ? (
                  ...
                  <div className="space-y-2">
                    ...
                    {(() => {
                      const allItems = useSqlRows
                        ? filteredPickerRows.map((row) => row.property)
                        : filteredProperties;
                      const byMode =
                        listingMode === 'all'
                          ? allItems
                      ...
                      const saleItems =
                        listingMode === 'all' ? allItems.filter((p) => isPropertyForSale(p)) : [];
                      const rentItems =
                        listingMode === 'all' ? allItems.filter((p) => !isPropertyForSale(p)) : [];

                      const renderItem = (p: العقارات_tbl) => {
                        const sqlRow = useSqlRows
                          ? sqlRowByPropertyId.get(String(p.رقم_العقار || ''))
                          : undefined;
                        const sqlActiveRec = isRecord(sqlRow?.active) ? sqlRow.active : null;

                        const sqlContractId = asTrimmedString(
                          getRecordProp(sqlActiveRec, 'contractId')
                        );
                        ...
                        const activeC: ActiveContractLike | null = sqlContractId
                          ...
                          : legacyActive
                            ? {
                                ...
                                guarantorId: legacyActive.رقم_الكفيل
                                  ? String(legacyActive.رقم_الكفيل)
                                  : undefined,
                        ...
                        const sqlTenantName = asTrimmedString(
                          getRecordProp(sqlActiveRec, 'tenantName')
                        );
                        const activeTenantId = asTrimmedString(activeC?.tenantId);
                        const tenantName =
                          sqlTenantName || (activeTenantId ? getTenantName(activeTenantId) : '');
                        const fallbackTenantName = activeTenantId
                          ? getTenantName(activeTenantId)
                          : '—';

                        const sqlGuarantorName = asTrimmedString(
                          getRecordProp(sqlActiveRec, 'guarantorName')
                        );
                        const sqlGuarantorPhone = asTrimmedString(
                          getRecordProp(sqlActiveRec, 'guarantorPhone')
                        );
                        const activeGuarantorId = asTrimmedString(activeC?.guarantorId);
                        const guarantorName =
                          sqlGuarantorName ||
                          (activeGuarantorId ? getGuarantorName(activeGuarantorId) : '');
                        const guarantorPhone =
                          sqlGuarantorPhone ||
                          (activeGuarantorId
                            ? asTrimmedString(peopleMap.get(activeGuarantorId)?.رقم_الهاتف)
                            : '');
                        const hasGuarantor = Boolean(sqlGuarantorName || activeGuarantorId);

                        const isSelected = value === p.رقم_العقار;
                        const ownerName =
                          (sqlRow?.ownerName ? String(sqlRow.ownerName) : '') ||
                          ...
                          >
                              ...
                              <div className="min-w-0">
                                ...
                                <div className="text-xs text-slate-500 dark:text-slate-400 mt-1 flex flex-wrap gap-x-3 gap-y-1">
                                  ...
                                  {hasActive ? (
                                    <span className="whitespace-nowrap" dir="ltr">
                                      عقد #{contractIdLabel(activeContractId)}
                                    </span>
                                  ) : null}
                                </div>
                                ...
                                {hasGuarantor ? (
                                  <div className="text-[11px] text-slate-500 dark:text-slate-400 mt-1 whitespace-normal break-words">
                                    كفيل: {guarantorName || '-'}
                                    {guarantorPhone ? (
                                      <span dir="ltr"> • {guarantorPhone}</span>
                                    ) : null}
                                  </div>
                                ) : null}
                              </div>
                            ...
                            <div className="flex items-start justify-between gap-3">
                              <div className="min-w-0 text-xs text-slate-600 dark:text-slate-300 flex items-start gap-2 flex-wrap">
                                ...
                                {area ? (
                                  <span
                                    className="text-[11px] opacity-80 font-mono tabular-nums whitespace-nowrap"
                                    dir="ltr"
                                  >
                                ...
                                {p.اسم_الحوض ? (
                                  <span className="text-[11px]">حوض: {p.اسم_الحوض}</span>
                                ) : null}
                                {p.رقم_قطعة ? (
                                  <span className="text-[11px]">قطعة: {p.رقم_قطعة}</span>
                                ) : null}
                                {p.رقم_لوحة ? (
                                  <span className="text-[11px]">لوحة: {p.رقم_لوحة}</span>
                                ) : null}
                              </div>
                      ...
                      if (listingMode === 'all') {
                        return (
                          <div className="space-y-3">
                            {saleItems.length ? (
                              <div>
                                <div className="px-2 text-[12px] font-bold text-emerald-700 dark:text-emerald-300">
                                  عقارات للبيع ({saleItems.length})
                                </div>
                            ...
                            {rentItems.length ? (
                              <div>
                                <div className="px-2 text-[12px] font-bold text-sky-700 dark:text-sky-300">
                                  عقارات للإيجار ({rentItems.length})
                                </div>
          ...
          <div className="p-4 border-t border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 flex justify-between items-center">
            <div className="text-xs text-slate-500">
              العدد الإجمالي:{' '}
              {useSqlRows ? pickerTotal || filteredPickerRows.length : filteredProperties.length}{' '}
              عقار
            </div>
            <button
              type="button"
