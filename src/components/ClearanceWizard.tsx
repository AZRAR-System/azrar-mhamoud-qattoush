...
export const ClearanceWizard: React.FC<ClearanceWizardProps> = ({ contract, onClose, onComplete }) => {
  ...
  const [addToBlacklist, setAddToBlacklist] = useState(false);
    ...
    const [legalNoticeMethod, setLegalNoticeMethod] = useState<'WhatsApp' | 'Email' | 'Print'>('Print');
  ...
  useEffect(() => {
    ...
    setSecurityDeposit(deposit);
        ...
        const preferredByTitle = templates.find(t => /فسخ|إنهاء|مخالصة/.test(String(t.title || '')))?.id;
    ...
    useEffect(() => {
        ...
        const generated = DbService.generateLegalNotice(selectedLegalTemplateId, contract.رقم_العقد, { date: clearanceDate });
        setLegalNoticeText(generated?.text || '');
    }, [selectedLegalTemplateId, contract.رقم_العقد, clearanceDate]);

  // --- CALCULATIONS ---
  const damagesTotal = damages.reduce((sum, d) => sum + d.cost, 0);
  const totalDebt = rentArrears + cleaningFee + damagesTotal + electricity.amountDue + water.amountDue;
  
  let depositAction: 'Return' | 'Execute' | 'ExecutePartial' = 'Return';
  if (totalDebt > 0) {
      if (totalDebt > securityDeposit) depositAction = 'Execute'; // Full + debt remains
  ...
  const removeDamage = (id: string) => {
    ...
    const handleSubmit = async () => {
        ...
        const res = DbService.terminateContract(contract.رقم_العقد, `مخالصة نهائية: ${finalNotes}`, record.date, record);
    ...
    <div className="flex flex-col h-full bg-gray-50 dark:bg-slate-900 overflow-hidden">
        ...
        <div className="bg-white dark:bg-slate-800 p-6 border-b border-gray-200 dark:border-slate-700 shadow-sm flex justify-between items-center">
            <div>
                ...
                <p className="text-sm text-slate-500">عقد #{formatContractNumberShort(contract.رقم_العقد)}</p>
            </div>
                    ...
                    <div key={idx} className={`flex flex-col items-center w-20`}>
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold transition-all
                            ${idx === step ? 'bg-indigo-600 text-white ring-4 ring-indigo-100' : 
                              idx < step ? 'bg-green-500 text-white' : 'bg-gray-200 text-gray-500'}
                        `}>
                            {idx < step ? <CheckCircle size={16}/> : idx + 1}
                        </div>
                        <span className={`text-[10px] mt-1 font-bold ${idx === step ? 'text-indigo-600' : 'text-gray-400'}`}>{label}</span>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-8 max-w-4xl mx-auto w-full">
                            ...
                            <div className="space-y-4">
                                ...
                                <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-900 rounded-lg cursor-pointer">
                                    <input 
                                        ...
                                        onChange={e => setElectricity({...electricity, paid: e.target.checked, amountDue: e.target.checked ? 0 : electricity.amountDue})}
                                    />
                            ...
                            <div className="space-y-4">
                                ...
                                <label className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-slate-900 rounded-lg cursor-pointer">
                                    <input 
                                        ...
                                        className="w-5 h-5 text-green-600"
                                        checked={water.paid}
                                        onChange={e => setWater({...water, paid: e.target.checked, amountDue: e.target.checked ? 0 : water.amountDue})}
                                    />
                                    <span className="font-bold">الفواتير مدفوعة بالكامل (براءة ذمة)</span>
                                </label>
                                {!water.paid && (
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
                        <table className="w-full text-right text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                ...
                                {damages.length === 0 && (
                                    <tr><td colSpan={3} className="p-6 text-center text-slate-400">لا يوجد أضرار مسجلة</td></tr>
                                )}
                            </tbody>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                        ...
                        <div className="bg-purple-50 dark:bg-purple-900/10 rounded-2xl border border-purple-200 dark:border-purple-800 p-6 flex flex-col justify-center">
                            ...
                            <div className={`p-4 rounded-xl border-2 text-center ${
                                depositAction === 'Return' ? 'bg-green-100 border-green-300 text-green-800' :
                                depositAction === 'Execute' ? 'bg-red-100 border-red-300 text-red-800' :
                                'bg-orange-100 border-orange-300 text-orange-800'
                            }`}>
                                <p className="text-xs font-bold uppercase mb-1">الإجراء الموصى به</p>
                                <h3 className="text-xl font-black mb-2">
                                    {depositAction === 'Return' ? 'إرجاع الورقة للمستأجر' : 
                                     depositAction === 'Execute' ? 'تنفيذ الورقة بالكامل' : 
                                     'تنفيذ جزئي / تسوية'}
                                </h3>
            ...
            {step === 4 && (
                <div className="space-y-6 animate-fade-in text-center max-w-lg mx-auto">
                    ...
                    <h3 className="text-2xl font-bold text-slate-800 dark:text-white">جاهز لإتمام المخالصة</h3>
                    ...
                    <div className="bg-gray-50 dark:bg-slate-800 text-right p-6 rounded-xl space-y-4 border border-gray-200 dark:border-slate-700 text-sm">
                        <div className="flex items-center gap-3">
                            ...
                            <span>تحويل حالة العقد إلى: <b>مفسوخ / منتهي</b></span>
                        </div>
                        <div className="flex items-center gap-3">
                            <CheckCircle size={16} className="text-green-500"/>
                            <span>
                    ...
                    <label className="flex items-start gap-3 p-4 bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900 rounded-xl cursor-pointer text-right">
                        <input type="checkbox" className="mt-1 w-5 h-5 text-red-600" checked={addToBlacklist} onChange={e => setAddToBlacklist(e.target.checked)} />
                        <div>
                            <span className="font-bold text-red-700 dark:text-red-400 flex items-center gap-2"><ShieldAlert size={16}/> إدراج المستأجر في القائمة السوداء</span>
                    ...
                    <div className="bg-gray-50 dark:bg-slate-800 text-right p-4 rounded-xl space-y-3 border border-gray-200 dark:border-slate-700">
                        <div className="flex items-center justify-between gap-3">
                            <div className="font-bold text-slate-700 dark:text-slate-200">مسودة إشعار قانوني (اختياري)</div>
                        ...
                        <div className="grid grid-cols-1 gap-3">
                            <div>
                                <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">نوع الإخطار</label>
                                <select
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">طريقة الإرسال (للتوثيق فقط)</label>
                                <select
                                    className="w-full bg-white dark:bg-slate-900 border border-gray-300 dark:border-slate-600 rounded-lg px-3 py-2 text-sm"
                            ...
                            <div>
                                <label className="block text-xs font-bold text-slate-600 dark:text-slate-300 mb-1">نص الإخطار</label>
                                <textarea
