...
export const Maintenance: FC = () => {
          ...
          try {
            ...
            if (active?.رقم_المستاجر) {
              ...
              const meta = items.find(
                (x) => String(x?.contract?.رقم_العقد) === String(active.رقم_العقد)
              );
  ...
  const handleFinishTicket = async (ticketId: string) => {
    ...
    try {
      ...
      DbService.updateMaintenanceTicket(ticketId, {
        الحالة: 'مغلق',
        ملاحظات_الإنهاء: notes || undefined,
      } as unknown as Partial<تذاكر_الصيانة_tbl>);
      refreshData();
  ...
  const handleDeleteTicket = async (ticketId: string) => {
    ...
    try {
      const maybeDelete = (
        DbService as unknown as { deleteMaintenanceTicket?: (id: string) => unknown }
      ).deleteMaintenanceTicket;
      const res = maybeDelete?.(ticketId) as unknown as
  ...
  const getPropertyContext = (propId: string) => {
    ...
    if (isDesktopFast) {
      ...
      return {
        prop: { رقم_العقار: pid } as unknown as العقارات_tbl,
        owner: null,
        tenant: null,
        ownerName: '...',
        tenantName: null,
    ...
    const activeContract = pickBestTenancyContract(
      contracts.filter((c) => c.رقم_العقار === propId)
    );
    const tenant = activeContract
      ? people.find((p) => p.رقم_الشخص === activeContract.رقم_المستاجر)
      : null;
  ...
  const pageCount = useMemo(
    () => Math.max(1, Math.ceil((filteredTickets.length || 0) / pageSize)),
