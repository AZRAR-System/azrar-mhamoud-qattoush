...
const t = (s: string) => s;
import { الأشخاص_tbl, العقارات_tbl, عروض_البيع_tbl, عروض_الشراء_tbl, اتفاقيات_البيع_tbl, SalesType } from '@/types';
import {
  Plus,
  Briefcase,
  FileSignature,
...
const SalesCard: React.FC<{ item: عروض_البيع_tbl | اتفاقيات_البيع_tbl, type: 'listing' | 'agreement' | 'offer' }> = ({ item, type }) => {
  ...
  const _isAgreement = 'السعر_النهائي' in item;
...
export const Sales: React.FC = () => {
  ...
  const [_offers, setOffers] = useState<عروض_الشراء_tbl[]>([]);
