...
export const Properties: React.FC = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {(() => {
        const noProperties = isDesktopFast
          ? desktopCounts?.properties === 0 && !desktopLoading
          : properties.length === 0;
        const noResults = isDesktopFast
            ...
            <div className="grid grid-cols-1 gap-4 sm:grid-cols-[repeat(auto-fit,minmax(360px,1fr))]">
              {(isDesktopFast ? desktopRows : uiRows).map(
                (rowOrProperty: DesktopPropertyPickerItem | العقارات_tbl, idx: number) => {
                  ...
                  // NOTE: تحديد (مفروش/فارغ) يجب أن يعتمد على "صفة العقار" فقط.
                  // لا نربط ذلك بوجود/عدم وجود عقد.
                  ...
                  return (
                    <Card
                      ...
                      className={`group w-full animate-slide-up ${visualRingClass}`}
                    >
                          ...
                          <div className="rounded-xl border border-slate-200/60 dark:border-slate-800 bg-slate-50/70 dark:bg-slate-950/20 p-3">
                            <div className="grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-2 items-start">
                              ...
                              <div className="text-sm font-bold text-slate-900 dark:text-white whitespace-normal break-words leading-snug">
                                <span
                                  className={`inline-block w-2.5 h-2.5 rounded-full ${ownerColor.dot} align-middle ml-2`}
                                ></span>
                                {ownerName || '—'}
                              </div>
                              ...
                              (isDesktopFast
                                ...
                                : Boolean(activeLegacy?.رقم_الكفيل)) ? (
                                <>
